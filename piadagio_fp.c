...
static struct delayed_work piadagio_fp_wq_task;
static DECLARE_DELAYED_WORK(piadagio_fp_wq_task, piadagio_fp_i2c_update);
...
static char piadagio_fp_buffer_i2c_rw[I2C_BUFFER_LEN + 1];		// Structure to r/w i2c data (+1 for the null character from sprintf)
...
static unsigned long piadagio_fp_i2c_update_counter = 0;		// Update counter
...
int piadagio_fp_i2c_update_screen() {
	...
	if (piadagio_fp_i2c_update_screen_half == 0) {
		bytes_2_send = snprintf(&piadagio_fp_buffer_i2c_rw[0], (I2C_BUFFER_LEN + 1),	// +1 for the null character from sprintf
					"%c%c%c%.*s%.*s",					// Format: length + cmd + position + lines
					I2C_BUFFER_LEN,						// Buffer length
	...
	if (bytes_2_send == I2C_BUFFER_LEN) {
		mutex_lock(&data->update_lock);
		bytes_2_send = i2c_master_send(piadagio_fp_i2c_client, &piadagio_fp_buffer_i2c_rw[0], I2C_BUFFER_LEN);
		mutex_unlock(&data->update_lock);
		if (bytes_2_send == I2C_BUFFER_LEN) {
...
static void piadagio_fp_i2c_update(struct work_struct *work) {
	...
	if (piadagio_fp_i2c_update_do > 0) {					// Check whether to run an update
		piadagio_fp_i2c_update_counter++;				// Debug helper, to know if this rountine is being executed
	...
	if (piadagio_fp_wq_kill == 0) {
		queue_delayed_work(piadagio_fp_wq, &piadagio_fp_wq_task, task_delay);
	}
}
...
static ssize_t piadagio_fp_get_stats(struct device *dev, struct device_attribute *dev_attr, char * buf) {
	printd("%s\n", __FUNCTION__);
	// Copy the result back to buf
	return sprintf(buf, "Update counter: %lu\nUpdate retries counter: %lu\nUpdate error counter: %lu\n",
			piadagio_fp_i2c_update_counter,
			piadagio_fp_i2c_update_retries_counter,
			piadagio_fp_i2c_update_errors_counter);
}
...
static int piadagio_fp_probe(struct i2c_client *client, const struct i2c_device_id *id) {
	...
	queue_delayed_work(piadagio_fp_wq, &piadagio_fp_wq_task, 10);
...
static int piadagio_fp_remove(struct i2c_client * client) {
	...
	cancel_delayed_work(&piadagio_fp_wq_task);	// Cancel any new tasks
